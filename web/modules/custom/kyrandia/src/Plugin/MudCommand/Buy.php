    ...
    if ($loc->getTitle() == 'Location 9') {
      ...
      if (count($words) > 1) {
        ...
        if (array_key_exists($target, $values)) {
          ...
          if ($price > $playerGold) {
            $results[$actingPlayer->id()][] = $this->gameHandler->getMessage('BUYM00');
            ...
            $results[$actingPlayer->id()][] = $this->gameHandler->getMessage('BUYM02');
            // Subtract gold.
            $playerGold -= $price;
          ...
          // Doesn't have it.
          $results[$actingPlayer->id()][] = $this->gameHandler->getMessage('BUYM04');
          $othersMessage = sprintf($this->gameHandler->getMessage('BUYM01'), $actingPlayer->field_display_name->value, $profile->field_kyrandia_is_female->value ? 'her' : 'him');
          $this->gameHandler->sendMessageToOthersInLocation($actingPlayer, $loc, $othersMessage, $results);
        }
      }
